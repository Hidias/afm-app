...
DROP POLICY IF EXISTS "qualiopi_documents_all" ON qualiopi_documents;
CREATE POLICY "qualiopi_documents_all" ON qualiopi_documents
  FOR ALL TO authenticated USING (true) WITH CHECK (true);

CREATE OR REPLACE FUNCTION update_qualiopi_documents_updated_at()
RETURNS TRIGGER AS $$
BEGIN NEW.updated_at = NOW(); RETURN NEW; END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trigger_qualiopi_documents_updated_at ON qualiopi_documents;
CREATE TRIGGER trigger_qualiopi_documents_updated_at
  BEFORE UPDATE ON qualiopi_documents FOR EACH ROW
  EXECUTE FUNCTION update_qualiopi_documents_updated_at();
...
CREATE TABLE IF NOT EXISTS reclamations (
  ...
  reclamant_type TEXT CHECK (reclamant_type IN ('stagiaire', 'client', 'financeur', 'autre')),
  session_id UUID REFERENCES sessions(id),
  objet TEXT NOT NULL,
  description TEXT,
  statut TEXT DEFAULT 'nouveau' CHECK (statut IN ('nouveau', 'en_cours', 'resolu', 'clos')),
...
CREATE POLICY "reclamations_all" ON reclamations
  FOR ALL TO authenticated USING (true) WITH CHECK (true);
