...
export const useDataStore = create((set, get) => ({
  ...
  updateTraineeResult: async (sessionId, traineeId) => {
    ...
    const allPresent = attendances && attendances.length > 0 && 
      attendances.every(a => a.present === true)
    ...
    // 4. Mettre à jour session_trainees
    const { data, error } = await supabase
      .from('session_trainees')
      .update({ result })
      .eq('session_id', sessionId)
  ...
  fetchGlobalStats: async () => {
    ...
    if (completedIds.length === 0) {
      return { tauxSatisfaction: 0, tauxRecommandation: 0, tauxPresence: 0, tauxReponse: 0, tauxReussite: 0 }
    }
    ...
    // Taux de réussite (Acquis / Total sessions terminées)
    ...
    const acquiredCount = resultData?.filter(r => r.result === 'acquired').length || 0
    const tauxReussite = totalResults > 0 ? (acquiredCount / totalResults) * 100 : 0
