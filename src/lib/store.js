...
export const useDataStore = create((set, get) => ({
  ...
  createTrainer: async (trainer) => {
    // Ne garder que les champs de base
    const validTrainer = {
  ...
  fetchSessions: async () => {
    ...
    for (let s of sessions) {
      // Charger le cours
      if (s.course_id) {
        const { data: course } = await supabase.from('courses').select('id, code, title, duration_hours').eq('id', s.course_id).single()
        s.courses = course
      }
      // Charger le client
      if (s.client_id) {
        const { data: client } = await supabase.from('clients').select('id, name, address, postal_code, city').eq('id', s.client_id).single()
        s.clients = client
      }
      // Charger le formateur
      if (s.trainer_id) {
        const { data: trainer } = await supabase.from('trainers').select('id, first_name, last_name, email').eq('id', s.trainer_id).single()
        s.trainers = trainer
      }
    }
  ...
  getSession: async (id) => {
    ...
    if (data) {
      ...
      const { data: trainees } = await supabase
        ...
        .eq('session_id', id)
      data.session_trainees = trainees || []
  ...
  createSession: async (sessionData) => {
    ...
    // Ne garder que les colonnes qui existent dans ta table
    const session = {
      course_id: rest.course_id,
      client_id: rest.client_id,
      trainer_id: trainer_ids?.[0] || null, // Premier formateur
  ...
  updateSession: async (id, updates) => {
    const { trainer_ids, trainee_ids, is_intra, ...rest } = updates
    
    // Ne garder que les champs valides
    const sessionUpdates = {
      course_id: rest.course_id,
      client_id: rest.client_id,
      trainer_id: trainer_ids?.[0] || rest.trainer_id || null,
      start_date: rest.start_date,
      end_date: rest.end_date,
    }
    
    // Enlever les undefined
    Object.keys(sessionUpdates).forEach(k => sessionUpdates[k] === undefined && delete sessionUpdates[k])
