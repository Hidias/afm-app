...
export const useDataStore = create((set, get) => ({
  ...
  createSession: async (sessionData) => {
    ...
    // Ne garder que les champs essentiels pour éviter erreurs colonnes manquantes
    const session = {
  ...
  fetchQualiopiReport: async () => {
    ...
    const score = totalChecks > 0 ? Math.round(((totalChecks - issueCount) / totalChecks) * 100) : 0
