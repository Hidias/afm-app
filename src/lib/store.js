...
export const useDataStore = create((set, get) => ({
  ...
  fetchSessions: async () => {
    ...
    try {
      ...
      // Auto-compléter les sessions planifiées dont la date est passée
      const today = new Date()
      today.setHours(0, 0, 0, 0)
      const sessionsToComplete = (sessionsData || []).filter(s => {
      ...
      // Mettre à jour les sessions en base (en arrière-plan)
      for (const s of sessionsToComplete) {
        supabase.from('sessions').update({ status: 'completed' }).eq('id', s.id).then(() => {
          console.log(`Session ${s.reference} auto-complétée`)
        })
