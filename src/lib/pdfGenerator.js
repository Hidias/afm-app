...
function generateConvention(session, trainees = [], trainer = null, costs = []) {
  ...
  const contact = getSessionContact(session) // Contact pour cette session
  ...
  doc.text(`Représentée par : ${contact.name || ''}`, 20, y); y += 4
  if (contact.role) { doc.text(`Fonction : ${contact.role}`, 20, y); y += 4 }
  ...
  // Calculer le coût HT avec les coûts supplémentaires
  ...
  // Construire le texte de l'article 3 avec les coûts détaillés
  ...
  if (costDetails.length > 0) {
    ...
    article3Text += `\n\nCOÛT TOTAL DE LA FORMATION : ${coutTotalHT.toFixed(2)} € HT`
  } else {
    article3Text += `\n\nCoût total de la formation : ${coutTotalHT.toFixed(2)} € HT`
  }
...
function generateFicheRenseignements(session, trainee = null, isBlank = false) {
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.text('INFORMATIONS PERSONNELLES', 20, y + 6)
  y += 12
  ...
  doc.text('Nom :', 20, y)
  if (isBlank) {
    doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  } else {
    doc.text(trainee?.last_name?.toUpperCase() || '', fieldStart, y)
  }
  y += lineHeight
  
  // Prénom
  doc.text('Prénom :', 20, y)
  if (isBlank) {
    doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  } else {
    doc.text(trainee?.first_name || '', fieldStart, y)
  }
  y += lineHeight
  ...
  if (isBlank) {
    doc.text('__ / __ / ____', fieldStart, y)
  } else {
    doc.text(trainee?.birth_date ? formatDate(trainee.birth_date) : '', fieldStart, y)
  }
  y += lineHeight
  
  // Email
  doc.text('Adresse email :', 20, y)
  doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  if (!isBlank && trainee?.email) doc.text(trainee.email, fieldStart, y - 1)
  y += lineHeight
  
  // Téléphone
  doc.text('Téléphone :', 20, y)
  doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  if (!isBlank && trainee?.phone) doc.text(trainee.phone, fieldStart, y - 1)
  y += lineHeight
  
  // N° Sécurité sociale
  doc.text('N° Sécurité Sociale :', 20, y)
  doc.text('|__|__|__|__|__|__|__|__|__|__|__|__|__|__|__|', fieldStart, y)
  y += lineHeight + 5
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.text('INFORMATIONS PROFESSIONNELLES', 20, y + 6)
  y += 12
  ...
  // Poste exercé
  doc.text('Poste exercé :', 20, y)
  doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  y += lineHeight
  
  // Depuis quand
  doc.text('Dans ce poste depuis :', 20, y)
  doc.line(fieldStart, y, fieldStart + 60, y)
  y += lineHeight
  
  // Adresse entreprise
  doc.text('Adresse de l\'entreprise :', 20, y)
  doc.rect(fieldStart, y - 4, fieldWidth, 18)
  y += 20
  
  // Année dernière formation
  doc.text('Année de votre dernière', 20, y)
  doc.text('formation dans ce domaine :', 20, y + 4)
  doc.line(fieldStart, y + 2, fieldStart + 40, y + 2)
  y += lineHeight + 5
  
  // Plus haut diplôme
  doc.text('Plus haut diplôme obtenu :', 20, y)
  doc.line(fieldStart, y, fieldStart + fieldWidth, y)
  y += lineHeight + 5
  
  // Niveau de connaissances
  doc.text('Niveau de connaissance dans le domaine de la formation :', 20, y)
  y += 6
  const niveaux = ['Aucune', 'Faible', 'Moyen', 'Bon', 'Expert']
  let xNiv = 30
  niveaux.forEach((niv, idx) => {
    doc.rect(xNiv, y, 4, 4)
    doc.text(niv, xNiv + 6, y + 3)
    xNiv += 35
  })
  y += 15
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(10)
  doc.text('VOS BESOINS ET ATTENTES', 20, y + 6)
  y += 12
  
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(9)
  doc.text('Quels sont vos besoins spécifiques et vos attentes concernant cette formation ?', 20, y)
  y += 5
  
  // Zone de texte libre
  doc.rect(20, y, pw - 40, 50)
  y += 55
  
  // Date et signature
  doc.text(`Date : ${isBlank ? '__ / __ / ____' : formatDate(new Date())}`, 20, y)
  doc.text('Signature du stagiaire :', pw - 80, y)
  doc.rect(pw - 80, y + 3, 60, 18)
  
  addFooter(doc, 'AF-FICHE')
  return doc
}
