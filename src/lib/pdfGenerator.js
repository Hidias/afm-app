...
function generateCertificatContent(doc, session, trainee, trainer) {
  ...
  doc.text(`Fait à Concarneau, le ${formatDate(new Date())}`, 20, y)
  y += 6
  doc.text('Pour l\'organisme de formation,', 20, y)
  y += 12
  
  if (ORG.stamp_base64) { try { doc.addImage(ORG.stamp_base64, 'JPEG', 20, y, 40, 40) } catch(e) {} }
...
function generateConvocationContent(doc, session, trainee, trainer) {
  ...
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`Concarneau, le ${formatDate(new Date())}`, 150, y, { align: 'right' })
  y += 10
  ...
  doc.text(`Lieu : `, 20, y)
  doc.setFont('helvetica', 'normal')
  doc.text(`${session?.location || 'À définir'}`, 50, y)
  y += 5
  ...
  doc.text('Nous vous prions de bien vouloir vous munir d\'une pièce d\'identité le jour de la formation.', 20, y)
  y += 8
  doc.text('Restant à votre disposition, nous vous prions d\'agréer nos salutations distinguées.', 20, y)
  y += 15
...
function generateAttestationContent(doc, session, trainee, trainer) {
  ...
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`Je soussigné, ${ORG.dirigeant}, représentant légal de l'organisme de formation ${ORG.name},`, 20, y)
  y += 5
  doc.text(`enregistré sous le numéro ${ORG.nda},`, 20, y)
  y += 10
  ...
  doc.text('A suivi avec assiduité la formation :', 20, y)
  y += 8
  ...
  y += 5
  doc.text(`Soit une durée de ${course.duration_hours || course.duration || '7'} heures.`, 20, y)
  y += 5
  doc.text(`Lieu : ${session?.location || 'En entreprise'}`, 20, y)
  y += 15
