...
function generateCertificat(session, trainee, clientData, trainer) {
  ...
  // Signature et tampon - BIEN AU-DESSUS DU PIED DE PAGE
  ...
  // Tampon positionné à droite, sous le nom
  y += 5
  try {
...
function generateEmargement(session, trainees, trainer, attendances = []) {
  ...
  doc.autoTable({
    startY: y,
    head: [['Nom Prénom', 'N° Sécurité Sociale', 'Matin', 'Après-midi', 'Matin', 'Après-midi']],
    body: tableData,
    theme: 'grid',
  ...
  doc.text(`${ORG.name} - ${ORG.address} - ${DOC_CODES.emargement}-${APP_VERSION}`, pageWidth / 2, pageHeight - 8, { align: 'center' })
  doc.setTextColor(0, 0, 0)
