...
// ============================================================
// GÉNÉRER TOUS LES DOCUMENTS D'UN TYPE EN UN SEUL PDF
// ============================================================
export function downloadAllDocuments(docType, session, trainees, options = {}) {
  const { client = null, trainer = null, attendances = [] } = options
  ...
  trainees.forEach((trainee, index) => {
    ...
    switch (docType) {
      ...
      case 'evaluation':
        ...
        break
      default:
        throw new Error(`Type de document non supporté pour génération en lot: ${docType}`)
    }
    
    if (index === 0) {
      mainDoc = doc
    } else {
      // Ajouter les pages du document suivant au document principal
      const pageCount = doc.internal.getNumberOfPages()
      for (let i = 1; i <= pageCount; i++) {
        mainDoc.addPage()
        // Copier le contenu de la page
        const pageContent = doc.internal.pages[i]
        mainDoc.internal.pages[mainDoc.internal.getNumberOfPages()] = pageContent
      }
    }
  })
  
  // Alternative plus simple : générer chaque doc et les concaténer avec jsPDF
  // Malheureusement jsPDF ne supporte pas bien la fusion, donc on va générer un seul PDF avec toutes les pages
  ...
  trainees.forEach((trainee, index) => {
    ...
    // Générer le contenu directement dans finalDoc
    switch (docType) {
      case 'convocation':
        generateConvocationInDoc(finalDoc, session, trainee, trainer)
        break
      case 'attestation':
        generateAttestationInDoc(finalDoc, session, trainee, trainer)
        break
      case 'certificat':
        generateCertificatInDoc(finalDoc, session, trainee, client || session.clients, trainer)
        break
      case 'evaluation':
        generateEvaluationInDoc(finalDoc, session, trainee, trainer)
        break
    }
  })
...
// Fonctions helper pour générer dans un doc existant
function generateConvocationInDoc(doc, session, trainee, trainer) {
  const pageWidth = doc.internal.pageSize.getWidth()
  const course = session.courses || {}
  const client = session.clients || {}
  let y = addHeaderWithTitle(doc, 'CONVOCATION À LA FORMATION')
...
function generateAttestationInDoc(doc, session, trainee, trainer) {
  const pageWidth = doc.internal.pageSize.getWidth()
  const course = session.courses || {}
  const client = session.clients || {}
  let y = addHeaderWithTitle(doc, 'ATTESTATION DE PRÉSENCE')
...
function generateCertificatInDoc(doc, session, trainee, clientData, trainer) {
  const pageWidth = doc.internal.pageSize.getWidth()
  const course = session.courses || {}
  let y = addHeaderWithTitle(doc, 'CERTIFICAT DE RÉALISATION')
...
function generateEvaluationInDoc(doc, session, trainee, trainer) {
  const pageWidth = doc.internal.pageSize.getWidth()
  const course = session.courses || {}
  let y = addHeaderWithTitle(doc, "FICHE D'ÉVALUATION DE SATISFACTION")
