...
function generateConvocation(session, trainee, trainer = null) {
  ...
  doc.text(`${ORG.dirigeant}`, 75, y + 8)
  doc.text(`Dirigeant ${ORG.name}`, 75, y + 13)
...
function generateAttestation(session, trainee, trainer = null) {
  ...
  try { doc.addImage(STAMP_BASE64, 'JPEG', 20, y, 50, 18) } catch {}
  doc.text(ORG.dirigeant, 75, y + 10)
...
// ============================================================
// ÉVALUATION À CHAUD - AVEC CERCLES GRAPHIQUES
// ============================================================
function generateEvaluation(session, trainee = null, isBlank = false) {
  ...
  for (let i = 1; i <= 5; i++) {
    doc.rect(20 + labelW + (i - 1) * colW, y, colW, 8, 'F')
  }
  doc.setFontSize(9)
  doc.setFont('helvetica', 'bold')
  doc.text('Critère', 22, y + 5.5)
  for (let i = 1; i <= 5; i++) {
    doc.text(String(i), 20 + labelW + (i - 1) * colW + colW / 2, y + 5.5, { align: 'center' })
  }
  y += 8
...
function generateEvaluationFroid(session, trainee = null, isBlank = false) {
  ...
  questions.forEach(q => {
    ...
    doc.text(' Non', 59, y)
    drawCircle(doc, 85, y, 1.5, false)
    doc.text(' Partiellement', 89, y)
    y += 10
  })
...
function generateCertificatContent(doc, session, trainee, trainer) {
  ...
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text("d'une action de formation professionnelle", 105, y, { align: 'center' })
  y += 15
  
  doc.setFontSize(11)
  doc.text(`Je soussigné(e) ${ORG.dirigeant}, représentant légal de l'organisme de formation`, 20, y)
  y += 6
  doc.setFont('helvetica', 'bold')
  doc.text(`${ORG.name}`, 20, y)
  doc.setFont('helvetica', 'normal')
  y += 6
  doc.text(`Numéro de déclaration d'activité : ${ORG.nda}`, 20, y)
  y += 10
  ...
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(12)
  doc.text(`${trainee?.civility || 'M./Mme'} ${trainee?.first_name || ''} ${trainee?.last_name?.toUpperCase() || ''}`, 105, y, { align: 'center' })
  y += 8
  
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  if (client?.name) {
    doc.text(`salarié(e) de l'entreprise ${client.name}`, 105, y, { align: 'center' })
    y += 8
  }
  
  doc.text('a suivi l\'action de formation :', 20, y)
  y += 10
  
  doc.setFont('helvetica', 'bold')
  doc.setFontSize(13)
  doc.text(`« ${course.title || 'Formation'} »`, 105, y, { align: 'center' })
  y += 12
  
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  doc.text(`Dates : du ${formatDate(session?.start_date)} au ${formatDate(session?.end_date)}`, 20, y); y += 6
  doc.text(`Durée : ${course.duration_hours || course.duration || '7'} heures`, 20, y); y += 6
  doc.text(`Lieu : ${session?.location || 'En entreprise'}`, 20, y); y += 6
  if (trainer) { doc.text(`Formateur : ${trainer.first_name} ${trainer.last_name}`, 20, y); y += 6 }
  y += 10
  
  doc.setFont('helvetica', 'bold')
  doc.text('Résultat obtenu :', 20, y); y += 8
  drawCheckbox(doc, 25, y - 3, 4, true)
  doc.setFont('helvetica', 'normal')
  doc.text('  Acquis', 32, y)
  drawCheckbox(doc, 80, y - 3, 4, false)
  doc.text('  Non acquis', 87, y)
  y += 15
  
  doc.text(`Fait à ${session?.location?.split(',')[0] || 'Concarneau'}, le ${formatDate(new Date())}`, 20, y)
  y += 15
  doc.text('Cachet et signature :', 20, y)
  if (ORG.stamp_base64) { try { doc.addImage(ORG.stamp_base64, 'JPEG', 20, y + 3, 40, 40) } catch(e) {} }
...
function generateConvocationContent(doc, session, trainee, trainer) {
  ...
  doc.setFontSize(12)
  doc.setFont('helvetica', 'bold')
  doc.text('CONVOCATION À UNE FORMATION', 105, y, { align: 'center' })
  y += 15
  
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  doc.text(`${session?.location?.split(',')[0] || 'Concarneau'}, le ${formatDate(new Date())}`, 150, y, { align: 'right' })
  y += 15
  
  doc.text(`À l'attention de : ${trainee?.civility || 'M./Mme'} ${trainee?.first_name || ''} ${trainee?.last_name?.toUpperCase() || ''}`, 20, y)
  y += 6
  if (client?.name) { doc.text(`Entreprise : ${client.name}`, 20, y); y += 6 }
  y += 10
  
  doc.text(`${trainee?.civility || 'Madame, Monsieur'},`, 20, y); y += 10
  
  const intro = `Nous avons le plaisir de vous convier à la formation "${course.title || 'Formation'}" qui se déroulera selon les modalités suivantes :`
  doc.splitTextToSize(intro, 170).forEach(line => { doc.text(line, 20, y); y += 5 })
  y += 8
  
  const details = [
    ['Intitulé :', course.title || 'Formation'],
    ['Dates :', `Du ${formatDate(session?.start_date)} au ${formatDate(session?.end_date)}`],
    ['Horaires :', `${session?.start_time || '09:00'} - ${session?.end_time || '17:00'}`],
    ['Lieu :', session?.location || 'À définir'],
    ['Durée :', `${course.duration_hours || course.duration || '7'} heures`],
  ]
  if (trainer) details.push(['Formateur :', `${trainer.first_name} ${trainer.last_name}`])
  ...
  doc.text('Nous vous prions de bien vouloir vous munir d\'une pièce d\'identité.', 20, y); y += 8
  doc.text('Cordialement,', 20, y); y += 12
  doc.setFont('helvetica', 'bold')
  doc.text(ORG.dirigeant, 20, y)
  doc.setFont('helvetica', 'normal')
  doc.text(ORG.name, 20, y + 5)
  
  if (ORG.stamp_base64) { try { doc.addImage(ORG.stamp_base64, 'JPEG', 130, y - 15, 40, 40) } catch(e) {} }
...
function generateAttestationContent(doc, session, trainee, trainer) {
  ...
  doc.setFont('helvetica', 'normal')
  y += 6
  doc.text(`N° d'activité : ${ORG.nda}`, 20, y)
  y += 10
  ...
  doc.setFont('helvetica', 'normal')
  doc.setFontSize(11)
  if (client?.name) { doc.text(`salarié(e) de l'entreprise ${client.name}`, 105, y, { align: 'center' }); y += 8 }
  
  doc.text('a bien participé à l\'action de formation :', 20, y)
  y += 10
  ...
  doc.setFontSize(11)
  doc.text(`Dates : du ${formatDate(session?.start_date)} au ${formatDate(session?.end_date)}`, 20, y); y += 6
  doc.text(`Durée effective : ${course.duration_hours || course.duration || '7'} heures`, 20, y); y += 6
  doc.text(`Lieu : ${session?.location || 'En entreprise'}`, 20, y); y += 6
  if (trainer) { doc.text(`Formateur : ${trainer.first_name} ${trainer.last_name}`, 20, y); y += 6 }
  y += 15
  
  doc.text(`Fait à ${session?.location?.split(',')[0] || 'Concarneau'}, le ${formatDate(new Date())}`, 20, y); y += 10
  doc.text('Pour servir et valoir ce que de droit.', 20, y); y += 15
  doc.text('Cachet et signature :', 20, y)
  if (ORG.stamp_base64) { try { doc.addImage(ORG.stamp_base64, 'JPEG', 20, y + 3, 40, 40) } catch(e) {} }
...
function generateEvaluationContent(doc, session, trainee) {
  ...
  doc.setFontSize(14)
  doc.setFont('helvetica', 'bold')
  doc.text('QUESTIONNAIRE DE SATISFACTION', 105, y, { align: 'center' })
  doc.setFontSize(10)
  doc.setFont('helvetica', 'normal')
  y += 6
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(`Date : du ${formatDate(session?.start_date)} au ${formatDate(session?.end_date)}`, 20, y); y += 5
  if (trainee) { doc.text(`Stagiaire : ${trainee.first_name || ''} ${trainee.last_name?.toUpperCase() || ''}`, 20, y); y += 5 }
  y += 6
  
  doc.setFont('helvetica', 'bold')
  doc.text('Merci de noter chaque critère (1 à 5) :', 20, y); y += 8
  
  const criteres = ['Organisation générale', 'Supports pédagogiques', 'Compétences formateur', 'Clarté des explications', 'Réponses aux questions', 'Rythme de la formation', 'Conditions matérielles', 'Adéquation attentes']
...
function generateEvaluationFroidContent(doc, session, trainee) {
  ...
  doc.setFont('helvetica', 'normal')
  y += 6
  doc.text('(À compléter 90 jours après la formation)', 105, y, { align: 'center' })
  y += 12
  
  doc.setFont('helvetica', 'bold')
  doc.text(`Formation : ${course.title || 'Formation'}`, 20, y); y += 5
  doc.setFont('helvetica', 'normal')
  doc.text(`Date de la formation : du ${formatDate(session?.start_date)} au ${formatDate(session?.end_date)}`, 20, y); y += 5
  if (trainee) { doc.text(`Stagiaire : ${trainee.first_name || ''} ${trainee.last_name?.toUpperCase() || ''}`, 20, y); y += 5 }
  y += 10
  ...
  doc.setFont('helvetica', 'bold')
  doc.text('Veuillez évaluer les points suivants :', 20, y); y += 8
  
  questions.forEach((q, idx) => {
    doc.setFont('helvetica', 'normal')
    doc.text(`${idx + 1}. ${q}`, 20, y); y += 6
    for (let i = 1; i <= 5; i++) {
      drawCircle(doc, 25 + (i - 1) * 18, y, 1.5, false)
      doc.text(String(i), 29 + (i - 1) * 18, y)
    }
    y += 10
  })
