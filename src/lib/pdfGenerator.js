...
function addHeader(doc, sessionRef = null) {
  const pw = doc.internal.pageSize.getWidth()
  // Logo placeholder (texte si pas d'image)
...
// ============================================================
// CONVENTION DE FORMATION - MISE EN PAGE WORD EXACTE
// ============================================================
function generateConvention(session, trainees = [], trainer = null) {
  ...
  doc.text(`Durée (heures) : ${course.duration || ''}`, 20, y); y += 5
...
// ============================================================
// CONVOCATION - MISE EN PAGE WORD EXACTE
// ============================================================
function generateConvocation(session, trainee, trainer = null) {
  ...
  doc.text(`${trainee?.first_name || ''} ${trainee?.last_name?.toUpperCase() || ''}`, pw / 2, y, { align: 'center' }); y += 10
  
  doc.setFontSize(10); doc.setFont('helvetica', 'normal')
  doc.text('Merci de vous présenter à la session de formation selon les informations suivantes :', 20, y); y += 10
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(`Durée totale : ${course.duration || ''} heures`, 20, y); y += 8
  
  doc.setFont('helvetica', 'bold')
  doc.text(`Lieu de formation : ${session?.location || ''}`, 20, y); y += 6
  doc.setFont('helvetica', 'normal')
...
function generateEmargement(session, trainees = [], trainer = null) {
  ...
  doc.text("FEUILLE D'ÉMARGEMENT", pw / 2, y, { align: 'center' }); y += 10
  ...
  // PAS de signature image - à faire à la main
...
// ============================================================
// ATTESTATION DE PRÉSENCE - MISE EN PAGE WORD EXACTE
// ============================================================
function generateAttestation(session, trainee, trainer = null) {
  ...
  doc.setFontSize(10); doc.setFont('helvetica', 'normal')
  doc.text(`Je soussigné, ${ORG.dirigeant}, représentant l'organisme de formation ${ORG.name}, atteste que :`, 20, y, { maxWidth: 170 }); y += 12
  
  // Nom du stagiaire en gras
  doc.setFont('helvetica', 'bold'); doc.setFontSize(12)
  doc.text(`${trainee?.first_name || ''} ${trainee?.last_name?.toUpperCase() || ''}`, pw / 2, y, { align: 'center' }); y += 8
  ...
  doc.text(`Durée totale : ${course.duration || ''} heures`, 20, y); y += 8
  doc.text(`Lieu : ${session?.location || ''}`, 20, y); y += 8
  ...
  doc.text(`Nombre total d'heures de présence : ${course.duration || ''}`, 20, y); y += 12
...
// ============================================================
// CERTIFICAT DE RÉALISATION - MISE EN PAGE WORD EXACTE
// ============================================================
function generateCertificat(session, trainee, trainer = null) {
  ...
  doc.setFontSize(10); doc.setFont('helvetica', 'normal')
  doc.text(`Je soussigné, ${ORG.dirigeant}, représentant légal du dispensateur de l'action concourant au développement des compétences ${ORG.name}, ${ORG.address}`, 20, y, { maxWidth: 170 }); y += 14
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(`${course.title || ''}`, pw / 2, y, { align: 'center' }); y += 12
  
  doc.setFont('helvetica', 'bold')
  doc.text("Nature de l'action concourant au développement des compétences :", 20, y); y += 8
  
  // Cases à cocher - ☑ pour Action de formation, ☐ pour les autres (barrés)
  doc.setFont('helvetica', 'normal')
  doc.text('☑  Action de formation', 25, y); y += 6
  doc.setTextColor(150, 150, 150)
  doc.text('☐  Bilan de compétences', 25, y); y += 6
...
// ============================================================
// ÉVALUATION À CHAUD - TABLEAU 1-5 CENTRÉ
// ============================================================
function generateEvaluation(session, trainee = null, isBlank = false) {
  ...
  // Légende
  doc.setFont('helvetica', 'bold')
  doc.text('Merci de noter chaque critère :', 20, y); y += 6
  doc.setFont('helvetica', 'normal'); doc.setFontSize(8)
  doc.text('1 = Insuffisant   2 = Passable   3 = Satisfaisant   4 = Bien   5 = Très Satisfaisant', 20, y); y += 10
  ...
  criteres.forEach(c => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      doc.text('○', 20 + labelW + (i - 1) * colW + colW / 2, y + 5.5, { align: 'center' })
    }
    doc.text(c, 22, y + 5.5)
    y += 8
  })
...
function generateEvaluationFormateur(session = null, isBlank = false) {
  ...
  doc.text(`Formateur : ${isBlank ? '________________________' : ''}`, 20, y); y += 10
  
  // Légende
  doc.setFont('helvetica', 'bold')
  doc.text('Évaluez chaque critère :', 20, y); y += 6
  doc.setFont('helvetica', 'normal'); doc.setFontSize(8)
  doc.text('1 = Insuffisant   2 = Passable   3 = Satisfaisant   4 = Bien   5 = Très Satisfaisant', 20, y); y += 10
...
// ============================================================
// TEST DE POSITIONNEMENT - QUESTIONS PERSONNALISÉES
// ============================================================
function generatePositionnement(session, questions = [], isBlank = false) {
  const doc = new jsPDF()
  const pw = doc.internal.pageSize.getWidth()
  const ph = doc.internal.pageSize.getHeight()
  const course = session?.courses || {}
  ...
  doc.text(`Prénom : ${isBlank ? '________________________________' : ''}`, 110, y); y += 7
  doc.text(`Date : ${isBlank ? '___/___/______' : formatDate(new Date())}`, 20, y)
  doc.text(`Entreprise : ${isBlank ? '________________________________' : (session?.clients?.name || '')}`, 110, y); y += 12
  ...
  if (questions.length === 0) {
    ...
    questions.forEach((q, idx) => {
      ...
      if (q.question_type === 'qcm') {
        // QCM - 3 réponses avec cases
        if (q.option_a) { doc.text(`○  ${q.option_a}`, 25, y); y += 6 }
        if (q.option_b) { doc.text(`○  ${q.option_b}`, 25, y); y += 6 }
        if (q.option_c) { doc.text(`○  ${q.option_c}`, 25, y); y += 6 }
      } else {
...
function generateProgramme(session, trainer = null) {
  ...
  doc.setFont('helvetica', 'normal')
  doc.text(course.methods || 'Apports théoriques, exercices pratiques, mises en situation.', 25, y, { maxWidth: 165 }); y += 10
...
function generateAnalyseBesoin(session = null, isBlank = false) {
  ...
  sections.forEach(s => {
    ...
    doc.setFont('helvetica', 'normal'); doc.rect(20, y, 170, 22); y += 28
  })
  doc.text('Signature entreprise :', 20, y); doc.rect(20, y + 3, 60, 18)
  doc.text('Signature Access Formation :', 110, y); doc.rect(110, y + 3, 60, 18)
...
export function downloadDocument(docType, session, options = {}) {
  ...
  switch (docType) {
    ...
    case 'positionnement': doc = generatePositionnement(session, questions, isBlank); filename = isBlank ? 'Test_Positionnement_Vierge.pdf' : `Test_Positionnement_${ref}.pdf`; break
