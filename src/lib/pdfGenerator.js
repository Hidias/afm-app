...
function addHeader(doc, sessionRef = null, options = {}) {
  ...
  const { logoBase64, orgInfo } = options
  
  // Logo: soit image paramétrable, soit rectangle bleu par défaut
  if (logoBase64) {
    try {
      doc.addImage(logoBase64, 'PNG', 15, 8, 40, 20)
    } catch (e) {
  ...
  // Infos société (paramétrables ou par défaut)
  const org = orgInfo || ORG
  doc.setFontSize(8)
  doc.setFont('helvetica', 'normal')
  doc.setTextColor(0, 0, 0)
  doc.text((org.nameFull || org.name || ORG.nameFull).toUpperCase(), 60, 14)
  doc.text(org.address || ORG.address, 60, 19)
  doc.text(`Tél : ${org.phone || ORG.phone} - Email : ${org.email || ORG.email}`, 60, 24)
  doc.text(`SIRET : ${org.siret || ORG.siret} - NDA : ${org.nda || ORG.nda}`, 60, 29)
...
function generateCertificat(session, trainee, trainer = null) {
  ...
  y += 6
  doc.text(`Pour une durée de ${course.duration || ''} heures.`, 20, y)
  y += 12
