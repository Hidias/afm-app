...
export default function Trainees() {
  ...
  const addBulkRow = () => {
    setBulkTrainees([...bulkTrainees, { first_name: '', last_name: '', email: '', phone: '', client_id: '' }])
  }
  ...
  const handleBulkSave = async () => {
    ...
    for (let i = 0; i < validTrainees.length; i++) {
      ...
      const { error } = await createTrainee({
        ...
        client_id: trainee.client_id || null,
      })
      if (error) {
    ...
    setBulkTrainees([{ first_name: '', last_name: '', email: '', phone: '', client_id: '' }])
  ...
  return (
    <div className="space-y-6">
      ...
      {showBulkAdd && (
        <div className="fixed inset-0 z-50 overflow-y-auto">
          <div className="fixed inset-0 bg-black/50" onClick={() => setShowBulkAdd(false)} />
          <div className="relative min-h-full flex items-center justify-center p-4">
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
              <div className="flex items-center justify-between p-4 border-b">
                ...
                <button onClick={() => setShowBulkAdd(false)}><X className="w-5 h-5" /></button>
              </div>
              <div className="p-4 overflow-y-auto flex-1">
                <div className="mb-4">
                  ...
                  <select 
                    ...
                    value={bulkTrainees[0]?.client_id || ''}
                    onChange={(e) => setBulkTrainees(bulkTrainees.map(t => ({ ...t, client_id: e.target.value })))}
                  >
                    <option value="">Sélectionner...</option>
                    {clients.map(c => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
              ...
              <div className="flex justify-between items-center gap-2 p-4 border-t">
                ...
                <div className="flex gap-2">
                  <button onClick={() => setShowBulkAdd(false)} className="btn btn-secondary">Annuler</button>
