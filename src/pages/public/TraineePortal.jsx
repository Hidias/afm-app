...
import { 
  ...
  Loader2, Calendar, Star, MessageSquare, ExternalLink, Briefcase
} from 'lucide-react'
...
export default function TraineePortal() {
  ...
  const loadSession = async () => {
    try {
      ...
      // Déterminer les périodes requises selon le type de journée
      // 'full' = journée complète (matin + après-midi)
      // 'half' = demi-journée (matin seulement par défaut)
      let sessionPeriods = ['morning', 'afternoon']
      if (sessionData.day_type === 'half') {
        sessionPeriods = ['morning'] // Une seule signature par jour
      }
      
      console.log('Session day_type:', sessionData.day_type, '-> Periods:', sessionPeriods)
  ...
  const handleSelectTrainee = async (trainee) => {
    ...
    try {
      ...
      setInfoForm({
        ...
        csp: infoData?.csp || trainee.csp || '',
        job_title: infoData?.job_title || trainee.job_title || '',
      ...
      attendances?.forEach(a => {
        // La table utilise des colonnes morning/afternoon (booléens)
      ...
      console.log('infoData?.filled_at:', infoData?.filled_at)
      console.log('isLastDay():', isLastDay())
      console.log('evalData?.questionnaire_submitted:', evalData?.questionnaire_submitted)
      console.log('allTodayAttendanceDone(attMap):', allTodayAttendanceDone(attMap))
      console.log('attendanceData (attMap):', attMap)
      
      if (!infoData?.filled_at) {
        console.log('-> Step: info_sheet (fiche non remplie)')
        setCurrentStep('info_sheet')
      } else if (!allTodayAttendanceDone(attMap)) {
        // TOUJOURS vérifier d'abord si les présences du jour sont signées
        console.log('-> Step: attendance (présences du jour non complètes)')
        setCurrentStep('attendance')
      } else if (isLastDay() && evalData?.questionnaire_submitted) {
        // Évaluation déjà soumise ET dernier jour ET toutes présences signées
        const questionKeys = [
        ...
        console.log('-> Step: google/thank_you (éval soumise, moyenne:', average, ')')
        ...
        // Dernier jour, présences faites, mais évaluation pas encore soumise
        console.log('-> Step: evaluation (dernier jour, présences ok, éval à faire)')
        setCurrentStep('evaluation')
      } else {
        // Pas le dernier jour, mais présences du jour faites
        console.log('-> Step: thank_you (présences ok, pas dernier jour)')
  ...
  // Vérifie si les périodes du jour sont complètes (respecte matin/après-midi seulement)
  const allTodayAttendanceDone = (attData = attendanceData) => {
    const today = getTodayFormation()
    if (!today) return false
    const dateStr = format(today, 'yyyy-MM-dd')
    
    // Utiliser les périodes de la session (par défaut: matin + après-midi)
    const periods = session?.periods || ['morning', 'afternoon']
    
    // Vérifier que toutes les périodes requises sont signées
    return periods.every(period => attData[`${dateStr}_${period}`])
  }
  ...
  const handleSubmitInfoSheet = async (e) => {
    ...
    if (!infoForm.csp) {
      errors.csp = 'La catégorie socio-professionnelle est requise'
    }
    if (!infoForm.job_title.trim()) {
      errors.job_title = 'Le poste / fonction est requis'
    }
  ...
  const handleSignAttendance = async (period) => {
    const today = getTodayFormation()
    if (!today) {
      console.error('Erreur: pas de jour de formation aujourd\'hui')
      alert('Erreur: la formation n\'a pas lieu aujourd\'hui')
      return
    }
    ...
    console.log('=== Signature présence ===')
    console.log('Date:', dateStr, 'Period:', period, 'Key:', key)
    console.log('Déjà signé?', attendanceData[key])
    
    // Déjà signé ?
    if (attendanceData[key]) {
      console.log('-> Déjà signé, ignoré')
      return
    }
    ...
    try {
      ...
      console.log('Ligne existante:', existing)
      
      if (existing) {
        // Mettre à jour la ligne existante
        console.log('UPDATE de la colonne', period)
        const { error: updateError } = await supabase
          .from('attendance_halfdays')
          .update({ 
            [period]: true, // morning ou afternoon
        ...
        console.log('INSERT nouvelle ligne')
        const insertData = {
          session_id: session.id,
          trainee_id: selectedTrainee.id,
          date: dateStr,
        ...
        console.log('Données INSERT:', insertData)
      ...
      // Vérifier si toutes les périodes d'aujourd'hui sont signées (respecte demi-journée)
      const periods = session?.periods || ['morning', 'afternoon']
      const allPeriodsDone = periods.every(p => newAttendance[`${dateStr}_${p}`])
      
      console.log('Vérification périodes:', periods, 'Toutes signées:', allPeriodsDone)
        ...
        <div className="bg-white rounded-b-xl shadow-lg p-5">
          ...
          {currentStep === 'info_sheet' && selectedTrainee && (
            <form onSubmit={handleSubmitInfoSheet} className="space-y-5">
              ...
              <div className="space-y-3">
                <h3 className="font-medium text-gray-900 flex items-center gap-2 text-sm">
                  <Briefcase className="w-4 h-4" /> Situation professionnelle
                </h3>
                ...
                <div>
                  <select
                    value={infoForm.csp}
                    onChange={(e) => {
                      setInfoForm({...infoForm, csp: e.target.value})
                      setFormErrors({...formErrors, csp: null})
                    }}
                    className={`w-full px-3 py-2 border rounded-lg text-sm ${formErrors.csp ? 'border-red-500' : ''}`}
                  >
                  ...
                  {formErrors.csp && <p className="text-xs text-red-500 mt-1">{formErrors.csp}</p>}
                </div>
                ...
                <div>
                  <input
                    ...
                    onChange={(e) => {
                      setInfoForm({...infoForm, job_title: e.target.value})
                      setFormErrors({...formErrors, job_title: null})
                    }}
                    className={`w-full px-3 py-2 border rounded-lg text-sm ${formErrors.job_title ? 'border-red-500' : ''}`}
                  />
                  {formErrors.job_title && <p className="text-xs text-red-500 mt-1">{formErrors.job_title}</p>}
                </div>
              </div>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-4">
                ...
                <p className="text-xs text-gray-500">
                  {session?.periods?.length === 1 
                    ? `Confirmez votre présence pour la ${session.periods[0] === 'morning' ? 'matinée' : 'après-midi'}`
                    : 'Confirmez votre présence pour chaque demi-journée'
                  }
                </p>
              </div>

              <div className="space-y-3">
                {/* Première demi-journée (ou seule si session half) */}
                {(session?.periods || ['morning', 'afternoon']).includes('morning') && (() => {
                  const dateStr = format(today, 'yyyy-MM-dd')
                  const morningKey = `${dateStr}_morning`
                  const morningDone = attendanceData[morningKey]
                  const isHalfDay = session?.day_type === 'half' || session?.periods?.length === 1
                  
                  return (
                    <div className={`p-4 rounded-lg border-2 ${morningDone ? 'border-green-500 bg-green-50' : 'border-gray-200'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <CheckCircle className={`w-6 h-6 ${morningDone ? 'text-green-600' : 'text-gray-400'}`} />
                          <div>
                            <p className="font-medium">{isHalfDay ? 'Émargement' : '1ère demi-journée'}</p>
                            <p className="text-xs text-gray-500">
                              {session?.start_time || '09:00'} - {isHalfDay ? (session?.end_time || '12:30') : '12:30'}
                            </p>
                          </div>
                        </div>
                        {morningDone ? (
                ...
                {/* Deuxième demi-journée - Affiché seulement pour journées complètes */}
                {(session?.periods || ['morning', 'afternoon']).includes('afternoon') && (() => {
                  const dateStr = format(today, 'yyyy-MM-dd')
                  const morningKey = `${dateStr}_morning`
                  ...
                  return (
                    <div className={`p-4 rounded-lg border-2 ${afternoonDone ? 'border-green-500 bg-green-50' : morningDone ? 'border-gray-200' : 'border-gray-100 bg-gray-50'}`}>
                      <div className="flex items-center justify-between">
                        <div className="flex items-center gap-3">
                          <CheckCircle className={`w-6 h-6 ${afternoonDone ? 'text-green-600' : morningDone ? 'text-gray-400' : 'text-gray-300'}`} />
                          <div>
                            <p className={`font-medium ${!morningDone ? 'text-gray-400' : ''}`}>2ème demi-journée</p>
                            <p className="text-xs text-gray-500">
                              {!morningDone ? 'Signez d\'abord la 1ère demi-journée' : `13:30 - ${session?.end_time || '17:00'}`}
                            </p>
                          </div>
                        </div>
                        {afternoonDone ? (
                          ...
                          <span className="px-4 py-2 bg-gray-200 text-gray-400 rounded-lg font-medium cursor-not-allowed">
                            Verrouillé
                          </span>
                        )}
                      </div>
