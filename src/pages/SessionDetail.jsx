...
export default function SessionDetail() {
  ...
  const handleTrainerEvalChange = async (field, value) => {
    const trainer = session.session_trainers?.[0]?.trainers
  ...
  const handleRemoveTrainee = async (traineeId) => {
    ...
    await removeTraineeFromSession(id, traineeId)
  ...
  const handleDownload = (docType, trainee = null) => {
    ...
    downloadDocument(docType, session, { trainees: sessionTrainees, trainee, trainer })
  ...
  const handleDownloadAll = (docType) => {
    const trainer = session.session_trainers?.[0]?.trainers
    const sessionTrainees = session.session_trainees?.map(st => st.trainees) || []
    downloadAllDocuments(docType, session, sessionTrainees, { trainer })
    ...
    <div className="space-y-6">
      ...
      {activeTab === 'suivi' && (
        <div className="space-y-6">
          ...
          <div className="card">
            <h3 className="font-semibold mb-4 flex items-center gap-2"><ClipboardCheck className="w-5 h-5 text-blue-600" />Évaluations stagiaires</h3>
