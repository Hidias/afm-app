...
export default function SessionDetail() {
  ...
  // Filtres pour sélection stagiaires
  const [traineeFilterClient, setTraineeFilterClient] = useState('')
  const [traineeSortBy, setTraineeSortBy] = useState('name') // 'name', 'date', 'company'
  ...
  useEffect(() => {
    const loadData = async () => {
      await Promise.all([fetchSessions(), fetchTrainees(), fetchTrainers(), fetchClients()])
      setLoading(false)
    }
    loadData()
  }, [])
  ...
  // Filtrer et trier les stagiaires disponibles
  let availableTrainees = trainees.filter(t => !enrolledTraineeIds.includes(t.id))
  
  // Filtre par entreprise
  if (traineeFilterClient) {
    availableTrainees = availableTrainees.filter(t => t.client_id === traineeFilterClient)
  }
  
  // Tri
  availableTrainees = [...availableTrainees].sort((a, b) => {
    switch (traineeSortBy) {
      case 'name':
        return `${a.last_name} ${a.first_name}`.localeCompare(`${b.last_name} ${b.first_name}`)
      case 'date':
        return new Date(b.created_at || 0) - new Date(a.created_at || 0) // Plus récent en premier
      case 'company':
            ...
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-lg animate-fadeIn">
              ...
              <div className="p-4">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <div>
                    <label className="text-xs text-gray-500 mb-1 block">Filtrer par entreprise</label>
                    <select
                      value={traineeFilterClient}
                      onChange={(e) => setTraineeFilterClient(e.target.value)}
                  ...
                  <div>
                    ...
                    <select
                      value={traineeSortBy}
                      onChange={(e) => setTraineeSortBy(e.target.value)}
                      className="input text-sm"
                    >
                ...
                {availableTrainees.length === 0 ? (
                  <p className="text-sm text-gray-500 text-center py-4">
                    {traineeFilterClient 
                      ? 'Aucun stagiaire disponible pour cette entreprise.' 
                  ...
                  <div className="space-y-1 max-h-72 overflow-y-auto border rounded-lg">
                    {availableTrainees.map(trainee => (
                      <label key={trainee.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 cursor-pointer border-b last:border-b-0">
                        <input
                          type="checkbox"
                        ...
                        <div className="flex-1">
                          <p className="font-medium text-sm">{trainee.first_name} {trainee.last_name}</p>
                          <p className="text-xs text-gray-500">{trainee.clients?.name || 'Sans entreprise'}</p>
                        </div>
                      </label>
                    ))}
                  </div>
                ...
                <div className="flex justify-between items-center mt-4">
                  <span className="text-sm text-gray-500">
                    {availableTrainees.length} stagiaire(s) disponible(s)
                  </span>
