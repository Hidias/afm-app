                ...
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {sessionTrainees.map(trainee => {
                      ...
                      const presStatus = getTraineePresenceStatus(trainee.id)
                        ...
                        <tr key={trainee.id} className="border-b hover:bg-gray-50">
                          ...
                          {getObjectivesList().map((obj, idx) => {
                            ...
                            return (
                              <td key={idx} className="text-center py-3 px-2">
                                <button
                                  onClick={() => handleToggleObjective(trainee.id, idx, isValidated)}
                          ...
                          <td className="text-center py-3 px-2">
                            <span className={`px-2 py-1 rounded text-xs font-medium ${
                              ...
                              {presStatus.present}/{presStatus.total}
                            </span>
                          </td>
                          <td className="text-center py-3 px-2">
                            {result === 'acquired' && (
                      ...
                      {isEnabled && (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 text-sm">
                          {[
                            ...
                            { key: 'q2_organization', label: 'Accueil/Orga.' },
                            { key: 'q3_content', label: 'Pertinence contenu' },
                            { key: 'q4_materials', label: 'Supports péda.' },
                            { key: 'q5_trainer', label: 'Pédagogie formateur' },
                            { key: 'q6_duration', label: 'Rythme/Durée' },
                            { key: 'q7_conditions', label: 'Conditions mat.' },
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center py-2">Compétences mises en pratique</th>
                      <th className="text-center py-2">Objectifs atteints</th>
                      <th className="text-center py-2">Répondu aux besoins</th>
                        ...
                        <tr key={t.id} className="border-b">
                          ...
                          <td className="text-center py-3">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => handleColdEvalChange(t.id, 'skills_applied', true)} className={`px-2 py-1 rounded text-xs ${coldEval.skills_applied === true ? 'bg-green-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Oui</button>
                              <button onClick={() => handleColdEvalChange(t.id, 'skills_applied', false)} className={`px-2 py-1 rounded text-xs ${coldEval.skills_applied === false ? 'bg-red-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Non</button>
                            </div>
                          </td>
                          <td className="text-center py-3">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => handleColdEvalChange(t.id, 'objectives_met', true)} className={`px-2 py-1 rounded text-xs ${coldEval.objectives_met === true ? 'bg-green-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Oui</button>
                              <button onClick={() => handleColdEvalChange(t.id, 'objectives_met', false)} className={`px-2 py-1 rounded text-xs ${coldEval.objectives_met === false ? 'bg-red-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Non</button>
                            </div>
                          </td>
                          <td className="text-center py-3">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => handleColdEvalChange(t.id, 'needs_met', true)} className={`px-2 py-1 rounded text-xs ${coldEval.needs_met === true ? 'bg-green-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Oui</button>
                              <button onClick={() => handleColdEvalChange(t.id, 'needs_met', false)} className={`px-2 py-1 rounded text-xs ${coldEval.needs_met === false ? 'bg-red-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Non</button>
                            </div>
                          </td>
                          <td className="text-center py-3">
                            <div className="flex items-center justify-center gap-1">
                              <button onClick={() => handleColdEvalChange(t.id, 'job_impact', true)} className={`px-2 py-1 rounded text-xs ${coldEval.job_impact === true ? 'bg-green-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Oui</button>
                              <button onClick={() => handleColdEvalChange(t.id, 'job_impact', false)} className={`px-2 py-1 rounded text-xs ${coldEval.job_impact === false ? 'bg-red-500 text-white' : 'bg-gray-100'}`} disabled={!isEnabled}>Non</button>
                            </div>
                          </td>
                          <td className="text-center py-3">
                            <div className="flex items-center justify-center gap-1">
