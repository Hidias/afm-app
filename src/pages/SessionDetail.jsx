...
export default function SessionDetail() {
  ...
  const handleForceResult = async (traineeId, forcedResult) => {
    if (!confirm(`Voulez-vous forcer le résultat à "${forcedResult === 'acquired' ? 'ACQUIS' : 'NON ACQUIS'}" ?`)) return
    const result = await forceTraineeResult(session.id, traineeId, forcedResult)
    setTraineeResults(prev => ({ ...prev, [traineeId]: result.result }))
    toast.success(`Résultat forcé : ${forcedResult === 'acquired' ? 'ACQUIS' : 'NON ACQUIS'}`)
  }
                            ...
                            <div className="flex items-center justify-center gap-2">
                              ...
                              <div className="relative group">
                                ...
                                <div className="absolute right-0 mt-1 w-32 bg-white border rounded shadow-lg hidden group-hover:block z-10">
                                  <button 
                                    onClick={() => handleForceResult(trainee.id, 'acquired')}
                                    className="block w-full text-left px-3 py-2 text-xs hover:bg-green-50 text-green-700"
                                  >
                                    Forcer Acquis
                                  </button>
                                  <button 
                                    onClick={() => handleForceResult(trainee.id, 'not_acquired')}
