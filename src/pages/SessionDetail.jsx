                    ...
                    <div key={t.id} className="border rounded-lg p-4">
                      ...
                      {isEnabled && (
                        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 text-sm">
                          {[
                            { key: 'q1_objectives', label: 'Atteinte objectifs' },
                            { key: 'q2_content', label: 'Clarté contenu' },
                            { key: 'q3_trainer', label: 'Qualité formateur' },
                            { key: 'q4_methods', label: 'Méthodes péda.' },
                            { key: 'q5_materials', label: 'Supports fournis' },
                            { key: 'q6_organization', label: 'Organisation' },
                          ...
                          <div className="flex flex-col">
                            <span className="text-xs text-gray-500 mb-1">Note globale</span>
                            <div className="flex gap-1">
                              {[1,2,3,4,5].map(n => (
                                <button key={n} onClick={() => handleEvalChange(t.id, 'satisfaction_score', n)} className={`w-6 h-6 rounded text-xs ${eval_.satisfaction_score === n ? 'bg-orange-600 text-white' : 'bg-gray-100 hover:bg-gray-200'}`}>{n}</button>
                              ))}
                            </div>
                          </div>
                          <div className="flex flex-col">
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b">
                      ...
                      <th className="text-center py-2">Connaissances retenues</th>
                    ...
                    {sessionTrainees.map(t => {
                      ...
                      return (
                        <tr key={t.id} className="border-b">
                          ...
                          <td className="text-center py-3">
                            <select value={coldEval.knowledge_retained || ''} onChange={(e) => handleColdEvalChange(t.id, 'knowledge_retained', e.target.value || null)} className="border rounded px-2 py-1 w-16" disabled={!isEnabled}>
                              <option value="">-</option>
                              {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                            </select>
                          </td>
                          <td className="text-center py-3">
                            <select value={coldEval.skills_applied || ''} onChange={(e) => handleColdEvalChange(t.id, 'skills_applied', e.target.value || null)} className="border rounded px-2 py-1 w-16" disabled={!isEnabled}>
                              <option value="">-</option>
                              {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                            </select>
                          </td>
                          <td className="text-center py-3">
                            <select value={coldEval.objectives_met || ''} onChange={(e) => handleColdEvalChange(t.id, 'objectives_met', e.target.value || null)} className="border rounded px-2 py-1 w-16" disabled={!isEnabled}>
                              <option value="">-</option>
                              {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                            </select>
                          </td>
                          <td className="text-center py-3">
                            <select value={coldEval.job_impact || ''} onChange={(e) => handleColdEvalChange(t.id, 'job_impact', e.target.value || null)} className="border rounded px-2 py-1 w-16" disabled={!isEnabled}>
                              <option value="">-</option>
                              {[1,2,3,4,5].map(n => <option key={n} value={n}>{n}</option>)}
                            </select>
                          </td>
