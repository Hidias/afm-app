...
export default function Dashboard() {
  ...
  const loadData = async () => {
    ...
    if (alerts && alerts.length > 0) {
      ...
      const [sessionsResult, traineesResult] = await Promise.all([
        sessionIds.length > 0 
          ? supabase.from('sessions').select('id, reference, courses(name)').in('id', sessionIds)
          : { data: [] },
        traineeIds.length > 0
  ...
  return (
    <div className="space-y-6">
      ...
      {(sessionsWithoutTrainer.length > 0 || nonConformites.length > 0 || sessionsInProgress.length > 0 || (purgeStats && purgeStats.trainees_to_purge > 0)) && (
      ...
      <div className="text-center text-xs text-gray-400 p-4 bg-primary-500/5 rounded-lg border border-primary-500/10">
        <p className="font-medium text-primary-600">Access Campus - Version 2.5.18</p>
