...
export default function Indicateurs() {
  ...
  const calcHotStats = () => {
    ...
    // Utiliser satisfaction_score comme note principale
    const satisfactionScores = filteredHotEvals
      .map(e => e.satisfaction_score)
      .filter(v => v !== null && v !== undefined)
    
    const avgSatisfaction = satisfactionScores.length > 0 
      ? (satisfactionScores.reduce((a, b) => a + Number(b), 0) / satisfactionScores.length).toFixed(1)
      : 0
  ...
  const globalScore = () => {
    let total = 0, count = 0
    if (hotStats && parseFloat(hotStats.avgSatisfaction) > 0) {
      total += parseFloat(hotStats.avgSatisfaction)
      count++
    }
    ...
    if (coldStats && coldStats.stats.length > 0) {
                  ...
                  <div className="flex items-center gap-4">
                    <div className="flex-1">
                      <div className="flex justify-between mb-1">
                        <span className="text-sm font-medium">Note de satisfaction</span>
                        <span className={`text-sm font-bold px-2 py-0.5 rounded ${ratingColor(parseFloat(hotStats.avgSatisfaction))}`}>
                          {hotStats.avgSatisfaction}/5
                        </span>
                      </div>
                      <div className="w-full bg-gray-200 rounded-full h-2">
                        <div 
                          className={`h-2 rounded-full ${ratingBg(parseFloat(hotStats.avgSatisfaction))}`}
                          style={{ width: `${(parseFloat(hotStats.avgSatisfaction) / 5) * 100}%` }}
                        />
                      </div>
                    </div>
                  </div>
