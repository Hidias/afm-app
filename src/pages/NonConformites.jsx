...
export default function NonConformites() {
  ...
  const filteredNC = nonConformites.filter(nc => {
    ...
    // Recherche par référence de session (SES-xxx ou juste les chiffres/lettres)
    let matchSession = true
    if (sessionSearch) {
      const searchTerm = sessionSearch.toUpperCase().replace('SES-', '')
      const session = sessions.find(s => s.id === nc.session_id)
      const sessionRef = session?.reference?.toUpperCase().replace('SES-', '') || ''
      matchSession = sessionRef.includes(searchTerm)
    }
  ...
  const handleStatusChange = async (id, newStatus) => {
    ...
    if (newStatus === 'closed') {
      ...
      updates.effectiveness_verified_by = currentUserName
    }
    if (newStatus === 'resolved') {
      updates.action_completed_at = new Date().toISOString()
      updates.action_responsible = currentUserName
    }
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">Suivi qualité - Amélioration continue</p>
        </div>
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="card p-8 text-center text-gray-500">
            {search || sessionSearch || statusFilter ? 'Aucun résultat' : 'Aucune non-conformité enregistrée'}
          </div>
        ) : (
          filteredNC.map((nc) => (
            <div key={nc.id} className="card hover:shadow-md transition-shadow">
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1 min-w-0">
                  ...
                  {nc.action_completed_at && nc.action_responsible && (
                    <div className="mt-2 text-sm text-gray-500">
                      Traité par: <span className="font-medium">{nc.action_responsible}</span> le {format(new Date(nc.action_completed_at), 'dd/MM/yyyy', { locale: fr })}
                    </div>
                  )}
                </div>
            ...
            <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl animate-fadeIn max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="p-4 space-y-4">
                ...
                <div className="grid md:grid-cols-3 gap-4">
                  <div>
                    <label className="label">Indicateur Qualiopi</label>
                    <input
                      type="text"
