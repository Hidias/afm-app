...
export default function TraineePortal() {
  ...
  const loadTraineeData = async () => {
    try {
      ...
      // Charger émargement depuis la table "attendances"
      ...
      attendanceRecords?.forEach(rec => {
        attendanceMap[`${rec.date}_${rec.period}`] = true
      })
    ...
    try {
      ...
      if (newValue) {
        ...
        if (existing) {
          ...
          if (error) {
            console.error('Erreur delete attendance:', error)
            throw error
          }
        }
      }
      ...
      const allPeriodsChecked = session.periods.every(p => {
        const k = `${today}_${p}`
        return (k === key ? newValue : attendanceData[k]) === true
      })
  ...
  const handleSubmitEvaluation = async (e) => {
    ...
    try {
      ...
      if (existing) {
        const { error } = await supabase.from('trainee_evaluations').update(evalData).eq('id', existing.id)
        if (error) {
          console.error('Erreur update evaluation:', error)
          throw error
        }
      } else {
        const { error } = await supabase.from('trainee_evaluations').insert(evalData)
        if (error) {
                    ...
                    return (
                      <div key={period} className="flex items-center justify-between p-4 border rounded-lg">
                        <span className="font-medium">
                          {period === 'am' ? '🌅 Matin (9h-12h)' : '🌆 Après-midi (13h-17h)'}
                        </span>
                        <button
                          type="button"
